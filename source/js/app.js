...
let numberText=document.getElementById('amount');
...
btnText.addEventListener('click',function(event){
    ...
    resault.innerHTML=''
    if(numberText.value!==''){
        let number=Number(numberText.value)
        if(numberText.value<10||numberText.value>0){
            
            console.log(number)


            // dummyText.forEach(function(text){

                
                

            //     // resault.innerHTML+=`<p>${text}</p><br>`

            // })


            for(i=0;i<=number;i++){
             newText+=`<p>${dummyText[i]}</p><br>`
                // newarray[0]+=dummyText[i]
                // newarray=[]
            }

            // console.log(newarray[0])
            resault.innerHTML+=newText  
            newText=''
            
            // console.log(newarray[0])
            
        }else{
            console.log("valu nit")
        }
...
// console.log(numberText,btnText)

// let sizeText=dummyText.length
